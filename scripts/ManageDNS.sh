...
ExecAndValidate()
{
  ...
  if [[ $random == 1 ]]; then 
    ...
    echo -e "$green  See /root/dnsfile.txt for a list of your domains.\n\n"
  else
    if [ ! -s $DNSconf ]
    then
      echo -e "\n\t$yellow No manually created DNS records found, script is exiting\n" 
      exit 0;
    fi
    ...
    echo -e "  See /root/dnsfile.txt for a list of your domains.\n\n"
  fi
}
DNSMenu
