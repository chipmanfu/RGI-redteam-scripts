...
Key Requirements:
  - Need to set up ssh keys between this server and the a.root server.

used for:
  -redirector using NGINX
...
Capabilities
  ...
  -Phishing attack platform
     This could use work, but for quick phishing/spamming attack capabilites the server
     has mutt and postfix installed.  Then in /root/scripts/phish/ there are a couple of
     example bash scripts for sending out individual attacks or loop through a email list.
 ...
 -Work that still needs done.
	...
	- Upgrading phishing to a better possibly web based front end solution.
