...
for i in {a..m} 
do
  echo -e "\t$green Working on $i.root"
  ...
  do
    ...
    #IP or nothing.
    resolveIP=$( nslookup $domain $i.root | awk -F':' '/^Address: / { matched = 1 } matched { print $2}' | xargs)
    #Checks is nslookup resolved an IP
    if [ -z $resolveIP ]
    then 
      echo -e "\t\t$red $domain lookup failed on root server $i.root"
    else 
      # Checks that the resolved IP matches the redteam-dns.lst IP
      if [ $resolveIP != $ip ]
      then
        echo -e "\t\t$red $domain shows $resolveIP and not $ip on root server $i.root"
      fi
    fi
  done<$dnsconf
done
